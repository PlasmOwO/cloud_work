...
def train_search(input_text): 
    ...
    texts = [ input_text, document1, document2, document3 ]


    vect = TfidfVectorizer()
    tf_idf = vect.fit_transform(texts).toarray()
    ...
    for id, document_tf_idf in enumerate(corpus):
        pearson_corr, _ = pearsonr(query_tf, document_tf_idf)
        if pearson_corr > 0.20:
            result = {"ID": id, "document": texts[id+1], "similarity": pearson_corr}
            #print(str(result))
            list_resultat.append(result)
    return list_resultat

# train_search("researchers noticed that the model")